/**
 * Augment analysis: pairwise lift model (I gronked that shhh), suggested build, seen pairs
...
//  Pairwise lift model (gronkeeeed)
/**
 * For each pair (A,B) seen together, compute:
 *   lift(A,B) = WR(A∩B) − mean(WR(A), WR(B))
...
export function buildPairLift(games, singleStats) {
    ...
    Object.entries(pairMap).forEach(([key, d]) => {
        const [a, b] = d.ids;
        const wrPair = d.wins / d.games * 100;
        const sA = singleStats[a], sB = singleStats[b];
        if (!sA || !sB) return;
        const wrA = sA.wins / sA.games * 100;
        const wrB = sB.wins / sB.games * 100;
        const lift = wrPair - (wrA + wrB) / 2;
        result[key] = { ids: d.ids, liftScore: lift, coGames: d.games, wrPair };
    });
    return result;
}
...
export function buildSuggestedAugBuild(games, singleStats, pairLift, totalChampGames) {
    ...
    const seen = Object.entries(singleStats).filter(([, d]) => d.games >= 2).map(([id, d]) => ({ id: Number(id), wr: d.wins / d.games * 100, games: d.games, rarity: getAugRarity(Number(id)) })).filter(a => a.rarity !== 'kUnknown');
    if (seen.length < 2) return null;
    const selected = [];
    const remaining = [...seen];
    const liftWeight = (coGames) => Math.min(1, coGames / 5);
    for (let slot = 0; slot < 4 && remaining.length > 0; slot++) {
        let bestScore = -Infinity, bestIdx = 0;
        remaining.forEach((cand, idx) => {
            let score = cand.wr;
            selected.forEach(sel => { const a = Math.min(cand.id, sel.id); const b = Math.max(cand.id, sel.id); const liftEntry = pairLift[`${a}|${b}`]; if (liftEntry) { score += liftEntry.liftScore * liftWeight(liftEntry.coGames); } });
    ...
    if (allSilver) {
        const higherRarity = seen.filter(a => a.rarity !== 'kSilver' && !selected.find(s => s.id === a.id)).sort((a, b) => b.wr - a.wr)[0];
        if (higherRarity) { const weakestIdx = selected.reduce((wIdx, a, i, arr) => a.wr < arr[wIdx].wr ? i : wIdx, 0); selected[weakestIdx] = higherRarity; }
    }
    selected.sort((a, b) => rarityOrder(b.rarity) - rarityOrder(a.rarity));
    const confidence = Math.min(100, Math.round(games.length / totalChampGames * 100));
    return { picks: selected, confidence, sampleGames: games.length };
}
...
export function buildSeenPairs(games, topN = 5, minGames = 2) {
    ...
    games.forEach(g => {
        const augs = [...new Set(g.augments.map(Number).filter(Boolean))];
        for (let i = 0; i < augs.length; i++) { for (let j = i + 1; j < augs.length; j++) { const a = augs[i], b = augs[j]; const key = a < b ? `${a}|${b}` : `${b}|${a}`; if (!map[key]) map[key] = { ids: a < b ? [a, b] : [b, a], games: 0, wins: 0 }; map[key].games++; if (g.win) map[key].wins++; } }
    });
    return Object.values(map).filter(d => d.games >= minGames).map(d => ({ ...d, winRate: d.wins / d.games * 100 })).sort((a, b) => b.winRate - a.winRate || b.games - a.games).slice(0, topN);
}
    ...
    pairs.forEach((pair, rank) => {
        ...
        card.innerHTML = `
            ...
            <div class="sc-combo-meta">
                <span class="${wrClass}">${pair.winRate.toFixed(1)}% WR</span>
...
export function renderSuggestedBuild(build) {
    ...
    const confClass = build.confidence >= 60 ? 'sc-conf-high' : build.confidence >= 30 ? 'sc-conf-mid' : 'sc-conf-low';
    wrap.innerHTML = `
        ...
        <div class="sc-suggested-footer">
            ...
            <span class="${confClass} sc-suggested-conf">
                ${build.confidence}% confidence · ${build.sampleGames} games
            </span>
        </div>
    `;
    return wrap;
...
export function buildAugSection(games, cAugStats, totalGames) {
    ...
    const augRows = Object.entries(cAugStats).map(([aId, d]) => {
        ...
        return { id: aId, name: info.name, icon: info.icon, rarity, ...d, winRate: d.wins / d.games * 100 };
    ...
    if (augRows.length === 0) {
        ...
        augSection.appendChild(createInteractiveTable([
            { label: 'Augment', key: 'name', render: r => { const rc = { kSilver: 'sc-aug-silver', kGold: 'sc-aug-gold', kPrismatic: 'sc-aug-prismatic' }[r.rarity] || ''; return `<div class="aram-icon-cell"><span class="sc-rarity-dot ${rc}"></span>${r.icon ? `<img src="${r.icon}" class="aram-icon">` : ''} ${r.name}</div>`; } },
