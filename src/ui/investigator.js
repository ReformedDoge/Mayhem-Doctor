...
import { startInvestigatorAnalysis, startSelfAnalysis } from "../analysis.js";
...
export function injectInvestigatorTab() {
  ...
  if (viewportMain.querySelector(`#${INVESTIGATOR_TAB_ID}`)) return;

  const subNav = viewportMain.querySelector(
  ...
  const sectionController = profileRoot.querySelector(
  ...
  subNav.querySelectorAll("lol-uikit-navigation-item").forEach((item) => {
    ...
    item.addEventListener("click", () =>
      deactivateInvestigatorTab(sectionController, panel, navItem),
    );
  });
}
...
export function injectMatchHistoryButton() {
  ...
  const injectionPoint = mainViewport
    ? mainViewport.querySelector(".match-history-left-title")
    : null;
  if (!injectionPoint || mainViewport.querySelector(".aram-controls-wrapper")) return;

  const wrapper = document.createElement("div");
  wrapper.className = "aram-controls-wrapper";

  const btn = document.createElement("button");
  btn.textContent = "Mayhem!";
  ...
  numInput.value = savedCount;
  numInput.className = "aram-number-input";
  ...
  btn.onclick = () => {
    ...
    const modalHost = createModalHost({ title: "My Mayhem Stats" });
    const modalContent = modalHost.content;
    modalContent.innerHTML =
      '<div class="aram-modal-close" title="Close">&times;</div><h2>Analyzing...</h2><p id="analysis-status">Initializing...</p>';
    modalContent.querySelector(".aram-modal-close").onclick = modalHost.close;
    ...
    startSelfAnalysis((update) => {
      ...
      if (update.finalStats) {
        displayStats(modalContent, update.finalStats, update.fullHistory, {
          onClose: modalHost.close,
          title: "My Mayhem Stats",
        });
      }
    }, count);
  };
